...
### Remaining
1. **F7** Post-migration legacy-config cleanup.
2. **F15** Diagnostics clear action + disclosure.
3. **F11** Decide: accept settings.json integrity risk or add HMAC.
4. Raise coverage ratchets as tests are added (bridge layer first).
5. External pentest + SIP parser fuzzing.
6. Revisit F3 (Authenticode) if distribution broadens beyond direct download.
