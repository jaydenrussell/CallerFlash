...
function createToastWindow(data) {
  ...
  setTimeout(() => {
    if (toastWindow && !toastWindow.isDestroyed() && !toastWindow.isVisible()) {
      console.console.log('[toast] safety timeout: forcing show');
      toastWindow.show();
      toastWindow.moveTop();
    }
  }, 200);
  ...
  toastWindow.webContents.on('did-fail-load', (_e, errorCode, errorDescription) => {
    console.console.log('[toast] LOAD FAILED:', errorCode, errorDescription);
  });
...
ipcMain.on('toast:show', (_event, data) => {
  console.console.log('[toast] toast:show received, data:', JSON.stringify(data || {}).substring(0, 100));
