...
impl SipClient {
    ...
    fn compute_digest_response(
        ...
        Self::md5_hex(&format!("{}:{}:{}:{}:{}:{}", ha1, nonce, nc, cnonce, qop, ha2))
    }
    ...
    fn build_sip_message(method: &str, uri: &str, headers: &HashMap<String, String>, body: &str) -> String {
    ...
    pub async fn start(&self, config: SipConfig) {
        ...
        tokio::spawn(async move {
            ...
            if is_tcp {
                handle.emit("sip:status", SipStatus {
            ...
            let auth_username = config.auth_username.clone().unwrap_or_else(|| config.username.clone());

            // Helper to build the REGISTER
            let build_register = |cseq_val: u32, call_id_ref: &str, auth_header: Option<&str>, expires_val: u32| -> String {
                let mut headers = HashMap::new();
                headers.insert("Via".to_string(), format!("SIP/2.0/UDP 127.0.0.1:5060;branch=z9hG4bK{}", Uuid::new_v4().to_string().split('-').next().unwrap_or("a")));
                headers.insert("Max-Forwards".to_string(), "70".to_string());
                headers.insert("From".to_string(), format!("<sip:{}@{}>;tag={}", config.username, config.server, Uuid::new_v4().to_string().split('-').next().unwrap_or("b")));
                headers.insert("To".to_string(), format!("<sip:{}@{}>", config.username, config.server));
                headers.insert("Call-ID".to_string(), call_id_ref.to_string());
                headers.insert("CSeq".to_string(), format!("{} REGISTER", cseq_val));
                headers.insert("Contact".to_string(), format!("<sip:{}@127.0.0.1:5060>", config.username));
                ...
                Self::build_sip_message("REGISTER", &format!("sip:{}:{}", config.server, port), &headers, "")
            };
            ...
            let (len, _) = match tokio::time::timeout(std::time::Duration::from_secs(10), socket.recv_from(&mut buf)).await {
                Ok(Ok((len, addr))) => (len, addr),
                _ => {
                    handle.emit("sip:status", SipStatus {
                        status: "error".to_string(),
                        message: Some("Timeout waiting for REGISTER response".to_string()),
                    }).ok();
                    return;
                }
            };
            ...
            if status_code == 401 || status_code == 407 {
                let auth_type = if status_code == 401 { "WWW-Authenticate" } else { "Proxy-Authenticate" };
                ...
                if let Some(www_auth) = auth_header {
                    ...
                    if algorithm.to_uppercase() != "MD5" {
                        handle.emit("sip:status", SipStatus {
                            status: "error".to_string(),
                            message: Some(format!("Unsupported digest algorithm: {}", algorithm)),
                        }).ok();
                        return;
                    }
                    ...
                    let response = Self::compute_digest_response(
                        &auth_username, &realm, &config.password,
                        "REGISTER", &uri, &nonce, &nc, &cnonce, &qop,
                    );
                    ...
                    if let Err(e) = socket.send_to(auth_reg.as_bytes(), server_addr).await {
                        handle.emit("sip:status", SipStatus {
                            status: "error".to_string(),
                            message: Some(format!("Failed to send authenticated REGISTER: {}", e)),
                        }).ok();
                        return;
                    }
                    cseq += 1;
                    ...
                    let (len, _) = match tokio::time::timeout(std::time::Duration::from_secs(10), socket.recv_from(&mut buf)).await {
                        ...
                        _ => {
                            handle.emit("sip:status", SipStatus {
                                status: "error".to_string(),
                                message: Some("Timeout waiting for auth REGISTER response".to_string()),
                            }).ok();
                            return;
                        }
                    };
                    ...
                    if status_code >= 200 && status_code < 300 {
                        *connected.lock().await = true;
                        ...
                        handle.emit("sip:status", SipStatus {
                            status: "error".to_string(),
                            message: Some(format!("Registration failed: {} {}", status_code, reason.trim())),
                        }).ok();
                        return;
                    }
                } else {
                ...
                *connected.lock().await = true;
                ...
                handle.emit("sip:status", SipStatus {
                    status: "error".to_string(),
                    message: Some(format!("Registration failed: {} {}", status_code, String::from_utf8_lossy(&buf[..len]).lines().next().unwrap_or("unknown"))),
                }).ok();
                return;
            }
            ...
            let _reregister_handle = tokio::spawn(async move {
                loop {
                    ...
                    hdrs.insert("Via".to_string(), format!("SIP/2.0/UDP 127.0.0.1:5060;branch=z9hG4bK{}", Uuid::new_v4().to_string().split('-').next().unwrap_or("r")));
                    hdrs.insert("Max-Forwards".to_string(), "70".to_string());
                    hdrs.insert("From".to_string(), format!("<sip:{}@{}>;tag={}", config_clone.username, config_clone.server, Uuid::new_v4().to_string().split('-').next().unwrap_or("f")));
                    hdrs.insert("To".to_string(), format!("<sip:{}@{}>", config_clone.username, config_clone.server));
                    hdrs.insert("Call-ID".to_string(), call_id_clone.clone());
                    hdrs.insert("CSeq".to_string(), format!("{} REGISTER", cseq_clone));
                    hdrs.insert("Contact".to_string(), format!("<sip:{}@127.0.0.1:5060>", config_clone.username));
                    hdrs.insert("Expires".to_string(), expiry.to_string());
                    hdrs.insert("User-Agent".to_string(), "CallerFlash".to_string());
                    let msg = Self::build_sip_message("REGISTER", &format!("sip:{}:{}", config_clone.server, port), &hdrs, "");
                    cseq_clone += 1;
